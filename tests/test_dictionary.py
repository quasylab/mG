...
class LegacyLambdasDictionaryTest(tf.test.TestCase):
    def test_simple_function(self):
        my_dict = FunctionDict({'add': PsiLocal(lambda x: tf.math.add(x, 1)), 'sub': PsiLocal(lambda x: tf.math.subtract(x, 1))})
    ...
    def test_function_generator(self):
        ...
        my_dict['add'] = lambda x: PsiLocal(lambda y: tf.math.add(y, int(x)))
...
class LambdasDictionaryTest(tf.test.TestCase):
    def test_simple_function(self):
        my_dict = FunctionDict({'add': PsiLocal.make(lambda x: tf.math.add(x, 1)), 'sub': PsiLocal.make(lambda x: tf.math.subtract(x, 1))})
        self.assertIsInstance(my_dict['add'], PsiLocal)
        self.assertIsInstance(my_dict['sub'], PsiLocal)

    def test_function_generator(self):
        my_dict = FunctionDict({})
        my_dict['add'] = PsiLocal.make_parametrized(lambda x, y: tf.math.add(y, int(x)))
        self.assertIsInstance(my_dict['add[1]'], PsiLocal)
        self.assertEqual(my_dict['add[1]'](tf.constant(1)), tf.constant(2))

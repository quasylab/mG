...
class Loop(MessagePassing):
    ...
    def get_inputs(inputs):
        if len(inputs) == 2:
            ...
            e = None
            i = None
        elif len(inputs) == 3 and K.ndim(inputs[-1]) == 2:
            x, a, e = inputs
            i = None
        elif len(inputs) == 3 and K.ndim(inputs[-1]) == 1:
            x, a, i = inputs
            e = None
        else:
            x, a, e, i = inputs
    ...
    def propagate(self, x, a, e=None, i=None, **kwargs):
        # TODO: deal with i, a, e
        const_inputs = [a, e] if e is not None else [a]
        return tf.while_loop(
            cond=lambda var: tf.math.reduce_all(self.test([var] + const_inputs)),
